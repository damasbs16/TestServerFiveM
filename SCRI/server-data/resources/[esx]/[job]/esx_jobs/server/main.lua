					...
					if timeElapsed > data.jobItem[1].time then
						...
						for k,v in ipairs(data.jobItem) do
							...
							if v.name ~= _U('delivery') and itemQtty >= v.max then
								TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type 'error', text = _U('max_limit', v.name), style = { ['background-color'] = '#b55255d9', ['color'] = '#fff' } })
								playersWorking[playerId] = nil
							elseif v.requires ~= 'nothing' and requiredItemQtty <= 0 then
								TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type 'error', text = _U('not_enough', data.jobItem[1].requires_name), style = { ['background-color'] = '#b55255d9', ['color'] = '#fff' } })
								playersWorking[playerId] = nil
							else
								if v.name ~= _U('delivery') then
		...
		if cautionAmount <= Config.MaxCaution and cautionAmount > 0 then
			TriggerEvent('esx_addonaccount:getAccount', 'caution', xPlayer.identifier, function(account)
				if xPlayer.getAccount('bank').money >= cautionAmount then
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type 'inform', text = _U('bank_deposit_taken', ESX.Math.GroupDigits(cautionAmount)), style = { ['background-color'] = '#3370a5d9', ['color'] = '#ffffff' } })
					TriggerClientEvent('esx_jobs:spawnJobVehicle', xPlayer.source, spawnPoint, vehicle)
				else
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type 'error', text = _U('caution_afford', ESX.Math.GroupDigits(cautionAmount)), style = { ['background-color'] = '#b55255d9', ['color'] = '#fff' } })
				end
			end)
		end
	elseif cautionType == 'give_back' then
		if cautionAmount <= 1 and cautionAmount > 0 then
			TriggerEvent('esx_addonaccount:getAccount', 'caution', xPlayer.identifier, function(account)
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type 'inform', text = _U('bank_deposit_returned', ESX.Math.GroupDigits(toGive)), style = { ['background-color'] = '#3370a5d9', ['color'] = '#ffffff' } })
			end)
		end
	end
end)