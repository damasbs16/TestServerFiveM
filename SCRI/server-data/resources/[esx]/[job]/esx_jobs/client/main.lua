...
AddEventHandler('esx_jobs:action', function(job, zone, zoneIndex)
	...
	elseif zone.Type == 'work' then
		...
		if IsPedOnFoot(playerPed) then
			...
			TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('foot_work') })
		end
	elseif zone.Type == 'vehspawner' then
		...
		if jobObject and spawnPoint and vehicle and ESX.Game.IsSpawnPointClear(spawnPoint.Pos, 5.0) then
			...
			TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('spawn_blocked') })
		end
				...
				if v.Type == 'vehdelete' and v.Spawner == zone.Spawner then
					...
					if IsPedInAnyVehicle(playerPed, false) then
						...
						if playerPed == driverPed then
							...
							TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('not_your_vehicle') })
						end
...
function nextStep(gps)
	if gps ~= 0 then
		...
		TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('next_point') })
	end
end
