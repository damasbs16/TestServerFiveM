			...
			if authorizedVehicles then
				if #authorizedVehicles > 0 then
					...
					if #shopElements > 0 then
						...
						TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('garage_notauthorized') })
					end
				else
					TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('garage_notauthorized') })
				end
							...
							if data2.current.stored then
								...
								if foundSpawn then
									...
									ESX.Game.SpawnVehicle(data2.current.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
										...
										TriggerEvent('mythic_notify:client:SendAlert', { type = 'success', text = _U('garage_released') })
									end)
								end
							else
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('esx_policejob:storeNearbyVehicle', function(storeSuccess, foundNum)
		if storeSuccess then
			...
			IsBusy = false
			TriggerEvent('mythic_notify:client:SendAlert', { type = 'success', text = _U('garage_has_stored') })
		else
...
function GetAvailableVehicleSpawnPoint(station, part, partNum)
	...
	if found then
		...
		TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('vehicle_blocked') })
		return false
	end
end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_policejob:buyJobVehicle', function (bought)
					if bought then
						TriggerEvent('mythic_notify:client:SendAlert', { type = 'success', text = _U('vehicleshop_bought', data.current.name, ESX.Math.GroupDigits(data.current.price)) })
