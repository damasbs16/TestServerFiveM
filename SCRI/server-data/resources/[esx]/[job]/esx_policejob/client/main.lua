...
function setUniform(uniform, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		...
		if uniformObject then
			...
			TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('no_outfit') })
		end
	end)
end
			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('service_out') })
					end
				end, 'police')
			end
		end
				...
				if not isInService then
					...
					ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
						if not canTakeService then
							TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('service_max', inServiceCount, maxInService) })
						else
							awaitService = true
							...
							TriggerServerEvent('esx_service:notifyAllInService', notification, 'police')
							TriggerEvent('esx_policejob:updateBlip')
							TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('service_in') })
						end
					end, 'police')
				...
				elseif DoesEntityExist(vehicle) then
					...
					elseif action == 'hijack_vehicle' then
						if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.0) then
							...
							TriggerEvent('mythic_notify:client:SendAlert', { type = 'success', text = _U('vehicle_unlocked') })
						end
						...
						Citizen.CreateThread(function()
							while currentTask.busy do
								...
								if not DoesEntityExist(vehicle) and currentTask.busy then
									TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('impound_canceled_moved') })
					...
					TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('no_vehicles_nearby') })
				end
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if not data.value or length < 2 or length > 8 then
			TriggerEvent('mythic_notify:client:SendAlert', { type 'error', text = _U('search_database_error_invalid') })
		else
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(playerData)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerEvent('mythic_notify:client:SendAlert', { type 'inform', text = _U('licence_you_revoked', data.current.label, playerData.name) })
	...
	while true do
		...
		if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'police_actions') then
			...
			elseif playerInService then
				...
				TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('service_not') })
			end
		end

		if IsControlJustReleased(0, 38) and currentTask.busy then
			TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('impound_canceled') })
			ESX.ClearTimeout(currentTask.task)
...
function StartHandcuffTimer()
	...
	handcuffTimer.task = ESX.SetTimeout(Config.HandcuffTimer, function()
		TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('unrestrained_timer') })
		TriggerEvent('esx_policejob:unrestrain')
